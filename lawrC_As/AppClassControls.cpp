...
void Application::ProcessKeyReleased(sf::Event a_event)
{
	...
	case sf::Keyboard::F:
		...
		// Show the next octant when U is pressed
	case sf::Keyboard::U:
		++m_uOctantID;
		...
		// Show the previous octant if J is pressed
	case sf::Keyboard::J:
		--m_uOctantID;
		...
		// If I is pressed on the num keypad, clear the dimensions from all entities
	...
	case sf::Keyboard::I:
		...
		// If K is pressed on the num keypad, clear the dimensions from all entities
		// Delete the oct tree, and create a new oct tree with the inputted amount
	case sf::Keyboard::K:
...
#pragma region Camera Position
	...
	// Each nested if statement is a condition to keep steve within bounds
	// Up Arrow Key
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up))
	{
		...
		if (minecraftMaze[int(ApproxHalf(targetPos.x) + 9.5f)][int(ApproxHalf(targetPos.z) + 9.5f)] == 0)
		{
			targetPos.z += 1.f;
			bMoved = false;
		}
	...
	// Keep Steve within the bounds using the clamp method
	targetPos.x = glm::clamp(targetPos.x, -9.5f, 9.5f);
	targetPos.z = glm::clamp(targetPos.z, -9.5f, 9.5f);
#pragma endregion
}
//Joystick
